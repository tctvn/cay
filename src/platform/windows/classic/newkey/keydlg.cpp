...
const wchar_t * ExpandLabel[] = {
		L"Expand",
		L"Mở rộng"};

const wchar_t * ContractLabel[] = {
		L"Contract",
		L"Thu nhỏ"};

DlgItemInfo VnDlgItems[] = {
    {IDC_FREE_MARKING,		(wchar_t*)L"Cho phép gõ dấu tự do, móc ở cuối từ"},
    {IDC_TONE_CTRL,			(wchar_t*)L"Phải gõ dấu ngay sau nguyên âm"},
    {IDC_MODERN_STYLE,		(wchar_t*)L"Đặt dấu oà, uý (thay vì òa, úy)"},
	{IDC_VIET_GUI,			(wchar_t*)L"Giao diện tiếng Việt"},
    {IDC_SHOW_DLG,			(wchar_t*)L"Bật hội thoại này khi khởi động"},
    {IDC_RESET,				(wchar_t*)L"Mặc định"},
    {IDC_EXIT,				(wchar_t*)L"&Kết thúc"},
    {IDOK,					(wchar_t*)L"Đón&g"},
    {IDC_OPEN_HELP,			(wchar_t*)L"&Hướng dẫn"},
    {IDC_ABOUT,				(wchar_t*)L"Thông tin"},
    {IDC_CONTROL_GROUP,		(wchar_t*)L"Điều khiển"},
    {IDC_CHARSET_PROMPT,	(wchar_t*)L"Bảng mã:"},
    {IDC_SWITCHKEY_PROMPT,	(wchar_t*)L"Phím chuyển:"},
    {IDC_KEYMODE_PROMPT,	(wchar_t*)L"Kiểu gõ:"},
    {IDC_OPTION_GROUP,		(wchar_t*)L"Tùy chọn khác"},
	{IDC_AUTO_START,		(wchar_t*)L"Khởi động cùng Windows"},
	{IDC_MACRO_CHECK,		(wchar_t*)L"Cho phép gõ tắt"},
	{IDC_MACRO_BTN,			(wchar_t*)L"Bảng gõ tắt..."},
	{IDC_USE_CLIPBOARD,     (wchar_t*)L"Sử dụng clipboard cho unicode"}
	};
...
DlgItemInfo EnDlgItems[] = { // use generic string
	...
	{IDC_AUTO_START,		_T("Auto-run Cay Classic at boot time")},
...
{
	const wchar_t * expandStr = (UserPref.m_vietGUI)? ExpandLabel[1] : ExpandLabel[0];
	const wchar_t * contractStr = (UserPref.m_vietGUI)? ContractLabel[1] : ContractLabel[0];
	...
	setupLanguage(1, VnDlgItems, sizeof(VnDlgItems)/sizeof(DlgItemInfo), VIET_FONT_NAME, VIET_DLG_FONT_SIZE);
...
void CKeyDlg::updateUI(CUserPref *pPref)
{
	SendMessage(m_codeList, CB_SETCURSEL, pPref->m_codeTable, 0); 
	SendMessage(m_inputList, CB_SETCURSEL, pPref->m_inMethod, 0); 
	SendMessage(m_showCtrl, BM_SETCHECK, (pPref->m_showDlg)? BST_CHECKED : BST_UNCHECKED, 0);
	SendMessage(m_markCheck, BM_SETCHECK, (pPref->m_freeMarking)? BST_CHECKED : BST_UNCHECKED, 0);
	SendMessage(m_toneCtrl, BM_SETCHECK, (pPref->m_toneNextToVowel)? BST_CHECKED : BST_UNCHECKED, 0);
	SendMessage(m_modernStyleCheck, BM_SETCHECK, (pPref->m_modernStyle)? BST_CHECKED : BST_UNCHECKED, 0);
	SendMessage(m_swKeyBtn1, BM_SETCHECK, (pPref->m_switchKey==0)? BST_CHECKED : BST_UNCHECKED, 0);
	SendMessage(m_swKeyBtn2, BM_SETCHECK, (pPref->m_switchKey==1)? BST_CHECKED : BST_UNCHECKED, 0);
	SendDlgItemMessage(m_hWnd, IDC_VIET_GUI, BM_SETCHECK, (pPref->m_vietGUI)? BST_CHECKED : BST_UNCHECKED, 0);
	SendDlgItemMessage(m_hWnd, IDC_AUTO_START, BM_SETCHECK, (pPref->m_autoStart)? BST_CHECKED : BST_UNCHECKED, 0);
	Button_SetCheck(m_macroCheck, pPref->m_macroEnabled);
	Button_SetCheck(m_useUnicodeClipboardCheck, pPref->m_useUnicodeClipboard);
}
...
#else
		wcscpy(m_strExpand, ExpandLabel[langIdx]);
		wcscpy(m_strContract, ContractLabel[langIdx]);
#endif
