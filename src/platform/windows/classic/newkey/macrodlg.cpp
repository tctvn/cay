...
const wchar_t * ReplaceLabel[] = {
		L"Replace",
		L"&Sửa"};

const wchar_t * AddLabel[] = {
		L"&Add",
		L"&Thêm"};


DlgItemInfo VnMacroDlgItems[] = {
	{IDC_STATIC_REPLACE,	(wchar_t*)L"Thay thế:"},
	{IDC_STATIC_WITH,		(wchar_t*)L"Bởi:"},
	{IDOK,					(wchar_t*)L"&Lưu"},
	{IDCANCEL,				(wchar_t*)L"&Ngừng"},
	{IDC_DELETE,			(wchar_t*)L"&Xóa"},
	{IDC_NOTE,				(wchar_t*)L"Lưu ý: thông tin ở đây được thể hiện dưới dạng VIQR"},
	{IDC_ALWAYS_MACRO,      (wchar_t*)L"Cho phép gõ tắt cả khi tắt tiếng Việt"},
	{IDC_SELECT_FILE,       (wchar_t*)L"Chọn File..."},
	{IDC_SELECT_DEFAULT,    (wchar_t*)L"File mặc định"},
	{IDC_PATH_LABEL,        (wchar_t*)L"File gõ tắt:"}
	};
...
int CMacroDlg::init(HINSTANCE hInst, HWND hWndOwner)
{
	...
	setupLanguage(1, VnMacroDlgItems, sizeof(VnMacroDlgItems)/sizeof(DlgItemInfo), VIET_FONT_NAME, VIET_DLG_FONT_SIZE);
	return ret;
}
...
void CMacroDlg::applyLanguage(int langIdx)
{
	...
	if (langIdx >= 0 && langIdx < 2) {
		const wchar_t *text = m_isEditReplace ? ReplaceLabel[langIdx] : AddLabel[langIdx];
...
void CMacroDlg::setReplaceBtn(int replace)
{
	...
	const wchar_t *text = replace ? ReplaceLabel[m_langID] : AddLabel[m_langID];
