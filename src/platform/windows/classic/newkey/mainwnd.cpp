...
// Removed duplicate pSharedMem definition
...
LRESULT CMainWnd::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	*/
	...
	case WM_HOOK_FLY_CONVERT:
		{
			int convErrCode;
			ConvertClipboard(UnifiedCharsetList[UserPref.m_inConvCharset].id,
						     UnifiedCharsetList[UserPref.m_outConvCharset].id,
						     convErrCode);
			result = TRUE;
		}
		break;
	...
	case WM_COMMAND:
		...
		case ID_END_KEY:
			...
			break;
		case IDM_ABOUT:
			DoAbout(AppInstance, m_hWnd, UserPref.m_vietGUI);
			result = TRUE;
			break;
		case ID_TOOLKIT:
			onToolkit();
...
//------------------------------------------------------
extern void CayUpdateInputMethod(int method);
...
void CMainWnd::updateChanges(const CUserPref &pref)
{
	...
	if (pref.m_changes & PREF_CHANGE_METHOD) {
		CayUpdateInputMethod(pref.m_inMethod);
	}
...
#else
	ShellExecute(NULL, _T("open"), _T("https://github.com/tctvn/cay"), NULL, NULL, SW_SHOWNORMAL);
#endif
}
